...
public class UserServiceImpl implements UserService {
	...
	public void findUserByAdminUserId(String id) throws UserException {
		User user = userMapper.findAdminUserByUserId(id);
		if (user != null) {
			throw new UserException("帐号已经被注册");
		}
	}	
	...
	public User login(User user) throws UserException {
		...
		System.out.println("_user.getPwd()="+_user.getPwd()+"Utils.MD5(user.getPwd()))="+Utils.MD5(user.getPwd()));
	...
	public User adminLogin(User user) throws UserException {
		// 根据账号查找用户
		User _user = userMapper.findAdminUserByUserId(user.getId());
	...
	public void checkAdminEmail(String email) throws UserException{
		User user = userMapper.findAdminUserByEmail(email);
		if(Utils.isEmpty(user)){
			throw new UserException("该邮箱未注册管理员");
		}
	}
	...
	public void updateAdminPwd(String email, String pwd) throws UserException {
		User user = userMapper.findAdminUserByUserId(email);
		if (!Utils.isEmpty(user)) {
			user.setPwd(pwd);
			userMapper.updateAdminPwd(user);
		} else
			throw new UserException("该邮箱未注册");
	}
